...
public class CollectionUtilites {
    ...
    public static <T, U> U foldRightRec(List<T> list, U a, Function<T, Function<U, U>> fun) {
        ...
        return list.isEmpty() ? a
                : foldRightRec(returnTailElement(list), fun.apply(returnHeadElement(list)).apply(a), fun);
    ...
    public static <T> Function<T, T> composeViaLeft(List<Function<T, T>> list) {
        return x -> foldLeft(list, x, a -> b -> b.apply(a));
    }

    public static <T> Function<T, T> composeViaRight(List<Function<T, T>> list) {
        return x -> foldRightRec(list, x, a -> b -> a.apply(b));
    }
